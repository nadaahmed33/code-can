...
import 'page10understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';
class Page9understandClass extends StatefulWidget{
  State<Page9understandClass>createState()=> _pg();
}
class _pg extends State<Page9understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
            ...
            child: Column(
              children: [
                ...
                Column(
                  children: [
                    Row(
                      //(٩) ليه لازم الواحد يكشف على نظره قبل ما يدخل المدرسة ؟
                      mainAxisAlignment:MainAxisAlignment.center ,
                      children: [
                        Text("ليه لازم الواحد يكشف",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(٩)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("على نظره قبل ما يدخل",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                    Center(
                      child: Text("المدرسة"+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                  ],
                ),
                // Container(
                ...
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=9 ");
