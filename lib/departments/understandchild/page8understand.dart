...
import 'page9understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';
class Page8understandClass extends StatefulWidget{
  State<Page8understandClass>createState()=> _pg();
}
class _pg extends State<Page8understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
            ...
            child: Column(
              children: [
                ...
                Column(
                  children: [
                    Row(
                      //(٨) لما مركب تيجى تغرق ، ليه لازم ننجى الستات والأطفال الأول ؟
                      mainAxisAlignment:MainAxisAlignment.center ,
                      children: [
                        Text("لما مركب تيجى تغرق",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(٨)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("، ليه لازم ننجى الستات",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                    Center(
                      child: Text("والأطفال الأول"+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                ...
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=8 ");
