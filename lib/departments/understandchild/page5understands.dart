...
import 'page6understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';
class Page5understandClass extends StatefulWidget{
  State<Page5understandClass>createState()=> _pg();
}
class _pg extends State<Page5understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
            ...
            child: Column(
              children: [
                ...
                Column(
                  children: [
                    Row(
                      //(٥) ايه اللى تعمله لو شفت قطر جاى بسرعة والقضبان مقطوعة ؟
                      mainAxisAlignment:MainAxisAlignment.center ,
                      children: [
                        Text("ايه اللى تعمله لو",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(٥)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("شفت قطر جاى بسرعة",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                    Center(
                      child: Text("والقضبان مقطوعة"+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                  ],
                ),
                // Container(
                ...
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=5 ");
