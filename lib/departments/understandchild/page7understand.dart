...
import 'page8understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';
class Page7understandClass extends StatefulWidget{
  State<Page7understandClass>createState()=> _pg();
}
class _pg extends State<Page7understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
            ...
            child: Column(
              children: [
                ...
                Column(
                  children: [
                    Row(
                      //(٧)  ليه لازم يكون فيه قوانين ؟
                      mainAxisAlignment:MainAxisAlignment.center ,
                      children: [
                        Text("ليه لازم يكون فيه",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(٧)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("قوانين"+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                  ],
                ),
                // Container(
                ...
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=7 ");
