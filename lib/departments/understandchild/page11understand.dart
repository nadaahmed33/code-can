...
import 'page12understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';

class Page11understandClass extends StatefulWidget{
  State<Page11understandClass>createState()=> _pg();
}
class _pg extends State<Page11understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
            ...
            child: Column(
              children: [
                ...
                Column(
                  children: [
                    Row(
                      //(١١)  ليه لازم يتعمل إمتحان للى عاوزين يشتغلوا في الحكومة ؟
                      mainAxisAlignment:MainAxisAlignment.center ,
                      children: [
                        Text(" ليه لازم يتعمل ",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(١١)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("إمتحان للى عاوزين",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),

                    Center(
                      child: Text("يشتغلوا في الحكومة "+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                ...
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=11 ");
