...
import 'page14understand.dart';
import 'package:codecansecond/logicoperation/voice.dart';
import 'package:codecansecond/logicoperation/sqlflite2.dart';
class Page13understandClass extends StatefulWidget{
  State<Page13understandClass>createState()=> _pg();
}
class _pg extends State<Page13understandClass> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
                ...
                Column(
                  children: [
                    Row(
                      ...
                      children: [
                        Text("ليه لازم يكون فيه ",style: TextStyle(fontSize:30,color: Color(0xff543686) ),),
                        Text("(١٣)",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                      ],
                    ),
                    Center(
                      child: Text("محامى ووكيل نيابة في ",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                    Center(
                      child: Text(" معظم المحاكمات"+"؟",style: TextStyle(fontSize:30,color: Color(0xff543686)  )),
                    ),
                  ],
                ),
                GestureDetector(
                    onTap: ()async{
                      question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='u' AND id=13 ");
